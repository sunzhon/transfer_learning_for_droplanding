...
def open_datafile(args):
    ...
    for domain, domain_name in zip(domains, domains_name):
        if domain !='None':
            ...
            domain_dataset = pd.read_csv(domain_data_folder)
...
# creat folder to save test files
...
# creat folder to save training files
def create_training_files(model_object=None, hyperparams={'lr':0}, base_folder=os.path.join(RESULTS_PATH,'training_testing/')):
    ...
    if 'investigation_results_folder' in hyperparams.keys():
        if(hyperparams['investigation_results_folder']!=None):
            date_base_folder = os.path.join(base_folder,hyperparams['investigation_results_folder'])
        else:
    ...
    with torch.no_grad():
        for idx, (features, labels) in enumerate(test_data_loader):
            ...
            if(args.save_test):
                ...
                testing_folder = create_testing_files(args.training_folder)
...
def train(domain_data_loaders,  model, optimizer, lr_scheduler, n_batch, args):
    ...
    training_folder = create_training_files(hyperparams=vars(args))
